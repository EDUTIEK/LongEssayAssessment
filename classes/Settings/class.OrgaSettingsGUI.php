...
class OrgaSettingsGUI extends BaseGUI
{
    ...
    private function updateSettings(array $data): void
    {
        ...
        if (!empty($data['writing']['writing_limit'])) {
            ...
            if ($data['writing']['writing_limit']['hours_minutes'] instanceof \DateTimeInterface) {
                list($hours, $minutes) = explode(':', $data['writing']['writing_limit']['hours_minutes']->format('H:i'));
                $limit += (int) $hours * 60 + (int) $minutes;
            }
        }
        $orga_settings->setWritingLimitMinutes($limit > 0 ? $limit : null);

        $orga_settings->setStartPassword(empty($data['writing']['start_password']) ? null : (string) $data['writing']['start_password']);
        ...
        $orga_settings->setSolutionAvailableDate($data['review']['solution_available']['solution_available_date'] ?? null);
        ...
        $orga_settings->setResultAvailableDate($data['review']['result_available_type'][1]['result_available_date'] ?? null);
    ...
    private function buildForm(): Standard
    {
        ...
        $sections = [
            ...
            'type' => $factory->section($fields_type, $this->plugin->txt('type_settings')),
            'info' => $factory->section($fields_info, $this->plugin->txt('info_settings')),
            'writing' => $factory->section($fields_writing, $this->plugin->txt('writing_organisation')),
            'correction' => $factory->section($fields_correction, $this->plugin->txt('correction_organisation')),
            'review' => $factory->section(
                $fields_review,
