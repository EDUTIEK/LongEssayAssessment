...
class CorrectionSettingsGUI extends BaseGUI
{
    ...
    protected function editSettings()
    {
        ...
        if (isset($data)) {
            ...
            // Multi Correctors
            if (!$orga_settings->getMultiTasks() && !$this->fixation_gui->isDisabled('correctors')) {
            ...
            if (!$this->has_authorized_corrections && !$this->fixation_gui->isDisabled('rating_settings')) {
            ...
            if (!$this->has_authorized_corrections && !$this->fixation_gui->isDisabled('correction_functions')) {
    ...
    private function buildForm(): Standard
    {
        ...
        // Correctors
        if (!$orga_settings->getMultiTasks() && $this->fixation_gui->isVisible('correctors')) {
            $fields = [];
            ...
            $sections['correctors'] = $factory->section($fields, $this->plugin->txt('correctors_per_writer'))
                ->withDisabled($this->fixation_gui->isDisabled('correctors'));
        }
        ...
        // Rating
        if ($this->fixation_gui->isVisible('rating_settings')) {
            $fields = [];
            ...
            $sections['rating_settings'] = $factory->section($fields, $this->plugin->txt('rating_settings'))
                ->withDisabled($this->has_authorized_corrections || $this->fixation_gui->isDisabled('rating_settings'));
        }

        // Functions
        if ($this->fixation_gui->isVisible('correction_functions')) {
            $fields = [];
            ...
            $sections['correction_functions'] = $factory->section($fields, $this->plugin->txt('correction_functions'))
                ->withDisabled($this->has_authorized_corrections || $this->fixation_gui->isDisabled('correction_functions'));
        }
