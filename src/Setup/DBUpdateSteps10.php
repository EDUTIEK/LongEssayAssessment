...
class DBUpdateSteps10 implements \ilDatabaseUpdateSteps
{
    ...
    public function step_2(): void
    {
        ...
        foreach ($this->tableInfo() as $table => $fields) {
            $this->migrateTable($table, $fields, $foreign_keys);
        }
    }
    ...
    private function migrateTable(string $name, array $field_info, array $foreign_keys): void
    {
        $fields = array_keys($field_info);
        $quote_id = $this->db->quoteIdentifier(...);
        $quote = $this->db->quote(...);
        $join_table = fn(string $root, string $join_me, string $root_field, string $join_field): string => sprintf(
            'LEFT JOIN %s ON %s.%s = %s.%s',
            ...array_map($quote_id, [$join_me, $root, $root_field, $join_me, $join_field])
        );
        ...
        foreach ($field_info as $field => $info) {
            if (!isset($info['src_table'])) {
                $what[] = join(' AS ', [$quote($this->dbDefault($info['Type'] ?? 'int(11)'), $quote_id($field))]);
            } else {
                $join[] = $info['src_table'];
                $what[] = sprintf('%s.%s AS %s', $quote_id($info['src_table']), $quote_id($info['Field']), $quote_id($field));
            }
        }

        $join = array_unique($join);
        $main_src = $join[0];
        $join = array_map(fn($join_me) => $join_table($main_src, $join_me, ...$foreign_keys[$main_src][$join_me]), array_slice($join, 1));
    ...
    {
        return [
            'xlas_writer' => [['user_id', 'task_id'], ['task_id']],
            'xlas_corrector' => [['user_id', 'task_id'], ['task_id']],
            'xlas_writer_comment' => [['task_id']],
            'xlas_corrector_summary' => [['essay_id'], ['essay_id', 'corrector_id']],
            'xlas_corrector_comment' => [['essay_id']],
            'xlas_access_token' => [['user_id', 'essay_id'], ['user_id'], ['task_id'], ['valid_until']],
            'xlas_corrector_ass' => [['writer_id', 'corrector_id'], ['corrector_id', 'writer_id']],
            'xlas_time_extension' => [['writer_id', 'task_id'], ['task_id']],
            'xlas_grade_level' => [['object_id']],
            'xlas_rating_crit' => [['object_id']],
            'xlas_writer_history' => [['essay_id'], ['hash_before'], ['hash_after']],
            'xlas_resource' => [['task_id'], ['file_id']],
            'xlas_log_entry' => [['task_id']],
            'xlas_alert' => [['task_id'], ['writer_id']],
            'xlas_location' => [['task_id']],
            'xlas_essay' => [['uuid'], ['writer_id'], ['task_id'], ['location']],
            'xlas_essay_image' => [['essay_id']],
            'xlas_writer_notice' => [['task_id'], ['writer_id'], ['essay_id']],
            'xlas_crit_points' => [['rating_id', 'corr_comment_id'], ['corr_comment_id'], ['essay_id'], ['corrector_id']],
        ];
    }
